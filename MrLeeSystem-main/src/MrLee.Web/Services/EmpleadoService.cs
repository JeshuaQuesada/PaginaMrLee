...
namespace MrLee.Web.Services
{
    public class EmpleadoService
    {
        ...
        public async Task<(bool Ok, string Error)> CrearAsync(EmpleadoFormVm vm, int actorId, string actorEmail)
        {
            ...
            var codigo = vm.Codigo.Trim();

            if (await _db.Empleados.AnyAsync(e => e.Codigo == codigo))
                return (false, $"El código '{codigo}' ya está en uso. Ingrese un código diferente.");
        ...
        public async Task<(bool Ok, string Error)> EditarAsync(EmpleadoFormVm vm, int actorId, string actorEmail)
        {
            ...
            if (await _db.Empleados.AnyAsync(e => e.Codigo == codigo && e.Id != empleado.Id))
                return (false, $"El código '{codigo}' ya está en uso. Ingrese un código diferente.");
            ...
            await _audit.LogAsync(actorId, actorEmail, "EDITAR", "Empleado", empleado.Id.ToString(),
                new { empleado.Codigo, empleado.Nombre, empleado.Apellido, empleado.Email, empleado.PuestoId, empleado.SucursalId });
