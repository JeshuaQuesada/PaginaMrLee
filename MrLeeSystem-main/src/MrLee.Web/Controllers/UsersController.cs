...
public class UsersController : Controller
{
    ...
    private Task<List<Role>> GetAssignableRolesAsync() =>
        _db.Roles
            .Where(r => r.IsActive && r.Name != "Cliente")
            .OrderBy(r => r.Name)
            .ToListAsync();

    private Task<bool> IsAssignableRoleAsync(int roleId) =>
        _db.Roles.AnyAsync(r => r.Id == roleId && r.IsActive && r.Name != "Cliente");
    ...
    {
        ViewBag.Roles = await GetAssignableRolesAsync();
        if (!await IsAssignableRoleAsync(vm.RoleId))
            ModelState.AddModelError(nameof(vm.RoleId), "Seleccione un rol válido.");
